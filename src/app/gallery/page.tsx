...
export default function Gallery() {
  ...
  const imagesCollection = useMemo(
    () => collection(getFirestore(), `gallery`) as CollectionReference<FirestoreImageData>,
    []
  );

  useEffect(() => {
    const unsubscribe = onSnapshot(
      query(imagesCollection, or(where('published', '==', true), where('uid', '==', uid))),
      {},
      async snapshot => {
        const images = Promise.all(
          snapshot.docs.map(async doc => {
            ...
            return {
              id: doc.id,
              ...data,
              image: data.image && (await getDownloadURL(ref(getStorage(), data.image))),
            };
          })
        );
        console.log(
  ...
  const uploadImage = useCallback(
    async (file: File) => {
      ...
      const newUploadStorageRef = ref(storageRef, `${uid}/${newUploadDocRef.id}-${file.name}`);
    ...
    <>
      ...
      <div className="flex-1 flex flex-col gap-4 overflow-y-scroll">
        {images.map(image => (
          <div
            key={image.id}
            className={`w-full my-4 px-4 group ${
              image.published ? '' : 'bg-red-900'
            }`}>
            <div className="border border-[#666] rounded-2xl flex gap-4 items-center pr-4 overflow-hidden">
              {image.image && <img src={image.image} alt="Image" width={200} />}
              <div className="flex-1">
                <MarkdownContainer markdown={image.output || ''}></MarkdownContainer>
              </div>
